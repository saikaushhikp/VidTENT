...
import torchvision.models.video as tvm_video   # [NEW] video model namespace
...
class VideoDataset(Dataset):
    ...
    Output tensor shape depends on the `video_model` flag:
      video_model=False  →  (T, C, H, W)   [default, 2D CNN pipeline]
      video_model=True   →  (C, T, H, W)   [NEW: 3D / video CNN pipeline]

    The (C, T, H, W) layout is required by torchvision video models
    (r3d_18, mc3_18, r2plus1d_18) which expect input as (B, C, T, H, W).
    """
    def __init__(self, video_paths, labels, num_frames, transform,
                 video_model: bool = False):   # [NEW] video_model flag
        self.video_paths = video_paths
        self.labels      = labels
        self.num_frames  = num_frames
        self.transform   = transform
        self.video_model = video_model         # [NEW]
    ...
    def __getitem__(self, idx):
        ...
        if not frames:
            ...
            dummy = torch.zeros(self.num_frames, 3,
                                ...
                                self.transform.transforms[1].size[0])
            # [NEW] Permute dummy too if using a video model
            if self.video_model:
                dummy = dummy.permute(1, 0, 2, 3)  # (T,C,H,W) → (C,T,H,W)
        ...
        # [NEW] Transpose to (C, T, H, W) for 3D video model input
        if self.video_model:
...
def build_model(num_classes: int, model_name=MODEL_NAME,
    ...
    ── 2D CNN path (unchanged) ──────────────────────────────────────────────────
      Loads a 2D backbone, strips its head, wraps it in FrameAggregator.
      Input contract: (B, T, C, H, W)

    ── [NEW] 3D video model path ────────────────────────────────────────────────
      If model_name is in VIDEO_MODELS, loads from torchvision.models.video,
      replaces model.fc to match num_classes, and returns the model directly.
      Input contract: (B, C, T, H, W)
    """
    print(f"\n[MODEL] Loading backbone: {model_name} (pretrained={pretrained})")

    # ── [NEW] 3D Video Model Branch ───────────────────────────────────────────
    if model_name in VIDEO_MODELS:
        weights = "DEFAULT" if pretrained else None
        constructor = getattr(tvm_video, model_name)
        net = constructor(weights=weights)

        # Replace the final fully-connected head for num_classes
        # All three supported models (r3d_18, mc3_18, r2plus1d_18) expose .fc
        in_features = net.fc.in_features
        net.fc = nn.Linear(in_features, num_classes)

        total_params = sum(p.numel() for p in net.parameters())
        trainable    = sum(p.numel() for p in net.parameters() if p.requires_grad)
        print(f"[MODEL] Video model: {model_name}  |  fc in_features: {in_features}")
        print(f"[MODEL] Total params: {total_params:,}  |  Trainable: {trainable:,}")
        return net
    # ── End of 3D Video Model Branch ─────────────────────────────────────────

    # ── 2D CNN Branch (original, unchanged) ──────────────────────────────────
...
class ViTTA:
    ...
    [NEW] video_model flag:
      When True, clips are permuted from (T,C,H,W) → (C,T,H,W) before
      being fed to the model, matching the (B,C,T,H,W) contract of 3D
      video backbones.
    """
    ...
    def predict(self, video_path: str, num_frames: int,
        ...
        [NEW] If self.video_model is True, each clip tensor is permuted from
        (T,C,H,W) to (C,T,H,W) so that unsqueeze(0) produces (1,C,T,H,W)
        as required by torchvision 3D video models.
        """
        ...
        for frames in clips_frames:
            ...
            # [NEW] Permute to (C, T, H, W) for 3D video models.
            # For 2D models the shape stays (T, C, H, W); unsqueeze(0)
            # gives (1, T, C, H, W) which FrameAggregator handles correctly.
            if self.video_model:
                clip = clip.permute(1, 0, 2, 3)  # (C, T, H, W)
        ...
        with torch.no_grad():
            ...
            for clip in clip_tensors:
                # unsqueeze(0):
                #   2D path → (1, T, C, H, W)  consumed by FrameAggregator
                #   3D path → (1, C, T, H, W)  consumed by video backbone
                logits = adapted(clip.unsqueeze(0).to(self.device))   # (1, num_classes)
                all_probs.append(torch.softmax(logits, dim=-1))
            avg_probs = torch.stack(all_probs, dim=0).mean(dim=0)     # (1, num_classes)
...
def main():
    ...
    print(f"  Model : {MODEL_NAME}  ({'3D video model' if IS_VIDEO_MODEL else '2D CNN'})")  # [NEW]
    ...
    # [NEW] Pass video_model=IS_VIDEO_MODEL so the dataset returns the correct
    #       tensor layout:
    #         IS_VIDEO_MODEL=False  →  (T, C, H, W)  for 2D CNN / FrameAggregator
    #         IS_VIDEO_MODEL=True   →  (C, T, H, W)  for 3D video backbone
    train_ds = VideoDataset(train_paths, train_labels, args.num_frames, train_tf,
                            video_model=IS_VIDEO_MODEL)
    ...
    if not args.ViTTA:
        ...
        # [NEW] Pass video_model=IS_VIDEO_MODEL so ViTTA.predict() permutes
        #       clips to (C,T,H,W) when using a 3D video backbone.
        vitta_engine = ViTTA(
            model      = model,
            ...
            device     = device,
            video_model= IS_VIDEO_MODEL,   # [NEW]
        )
